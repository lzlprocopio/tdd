package br.com.lzl.teste_de_unidade_mvn;

import java.util.Arrays;
import java.util.List;

import org.junit.Test;

import br.com.lzl.leilao.dominio.FiltroDeLances;
import br.com.lzl.leilao.dominio.Lance;
import br.com.lzl.leilao.dominio.Usuario;
import static org.junit.Assert.assertEquals;

public class FiltroDeLancesTest {

	@Test
	public void deveSelecionarLancesEntre1000E3000() {
		Usuario joao = new Usuario("Joao");

		FiltroDeLances filtro = new FiltroDeLances();
		List<Lance> resultado = filtro.filtra(Arrays.asList(
												new Lance(joao, 2000), 
												new Lance(joao, 1000),
												new Lance(joao, 3000), 
												new Lance(joao, 800)));

		assertEquals(1, resultado.size());
		assertEquals(2000, resultado.get(0).getValor(), 0.00001);
	}
	@Test
	public void deveSelecionarLancesAcimaDe5000(){
		Usuario joao = new Usuario("Joao");

		FiltroDeLances filtro = new FiltroDeLances();
		List<Lance> resultado = filtro.filtra(Arrays.asList(
												new Lance(joao, 5400), 
												new Lance(joao, 1000),
												new Lance(joao, 5000), 
												new Lance(joao, 5600)));

		assertEquals(2, resultado.size());
		assertEquals(5400, resultado.get(0).getValor(), 0.00001);
		assertEquals(5600, resultado.get(1).getValor(), 0.00001);
		
		
	}
	@Test
	public void naoDeveEncontrarNenhumLance(){Usuario joao = new Usuario("Joao");

	FiltroDeLances filtro = new FiltroDeLances();
	List<Lance> resultado = filtro.filtra(Arrays.asList(
											new Lance(joao, 4400), 
											new Lance(joao, 1000),
											new Lance(joao, 5000), 
											new Lance(joao, 4600)));

	assertEquals(0, resultado.size());
}

	@Test
	public void deveSelecionarLancesEntre500E700() {
		Usuario joao = new Usuario("Joao");

		FiltroDeLances filtro = new FiltroDeLances();
		List<Lance> resultado = filtro.filtra(
				Arrays.asList(
						new Lance(joao, 600), 
						new Lance(joao, 500), 
						new Lance(joao, 700), 
						new Lance(joao, 800)));

		assertEquals(1, resultado.size());
		assertEquals(600, resultado.get(0).getValor(), 0.00001);
	}
}